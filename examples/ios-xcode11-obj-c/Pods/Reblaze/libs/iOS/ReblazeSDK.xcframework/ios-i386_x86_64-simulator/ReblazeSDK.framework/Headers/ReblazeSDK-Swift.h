...
///
/// \param key The name of a field that contains a unique value for this user. (Example: UserName.)
///
/// \param uid The value for the field specified above. (For example, the user’s email address.)
///
/// \param shouldShowLogs Value indicating will logs be printed in debug console
///
/// \param intervalInSeconds interval in seconds when the events will be sent. Must be in range 12…300 seconds. If value specified to -1, this means no events will be send authomatically
///
+ (BOOL)startWith:(NSString * _Nonnull)address secret:(NSString * _Nonnull)secret key:(NSString * _Nonnull)key uid:(NSString * _Nonnull)uid shouldShowLogs:(BOOL)shouldShowLogs intervalInSeconds:(NSInteger)intervalInSeconds error:(NSError * _Nullable * _Nullable)error;
